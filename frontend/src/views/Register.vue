<template>
  <div class="max-w-md mx-4 sm:mx-auto bg-white border border-slate-100 shadow-sm p-10 rounded-[3rem] mt-12 relative overflow-hidden">
    <h2 class="text-3xl font-extrabold text-center mb-8 text-slate-900 relative z-10">Join PawHub</h2>
    <form @submit.prevent="handleRegister" class="relative z-10">
      <div class="mb-5">
        <label class="block text-slate-700 text-sm font-bold mb-2" for="name">Full Name</label>
        <input v-model="name" type="text" id="name" required class="block w-full rounded-2xl border-slate-200 shadow-sm focus:border-sky-500 focus:ring-sky-500 px-5 py-3 border bg-slate-50 transition-all" placeholder="Jane Doe">
      </div>
      <div class="mb-5">
      ...
      <div>
        <button class="w-full bg-orange-500 text-white font-extrabold py-4 px-6 rounded-2xl hover:bg-orange-600 transition-all shadow-md hover:-translate-y-0.5" type="submit" :disabled="loading">
          {{ loading ? 'Creating Account...' : 'Create Account' }}
        </button>
      </div>
      <div class="text-center mt-6 pt-6 border-t border-slate-100">
        <p class="text-sm text-slate-600 font-medium">Already have an account? <router-link to="/login" class="text-sky-600 hover:text-sky-800 font-bold ml-1 transition-colors">Sign in</router-link></p>
      </div>
    </form>
  </div>
</template>
