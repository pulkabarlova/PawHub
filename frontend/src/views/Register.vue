<template>
  ...
  >
    ...
    <form @submit.prevent="handleRegister" class="relative z-10">
      ...
      <div>
        <button
          class="w-full bg-orange-500 text-white font-extrabold py-4 px-6 rounded-2xl hover:bg-orange-600 transition-all shadow-md hover:-translate-y-0.5"
          type="submit"
          :disabled="loading"
        >
      ...
      <div class="text-center mt-6 pt-6 border-t border-slate-100">
        <p class="text-sm text-slate-600 font-medium">
          ...
          <router-link to="/login" class="text-sky-600 hover:text-sky-800 font-bold ml-1 transition-colors"
            >Sign in</router-link
          >
        </p>
      </div>
