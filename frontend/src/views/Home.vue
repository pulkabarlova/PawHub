      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-20 lg:pt-32 lg:pb-32">
        <div class="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          <div class="w-full lg:w-1/2 text-center lg:text-left relative z-10">
            <h1
              class="text-5xl md:text-6xl lg:text-7xl font-extrabold tracking-tight mb-8 text-slate-900 leading-[1.1]"
            >
              Find your <br class="hidden lg:block" />
              new <span class="text-sky-500">best friend.</span>
            </h1>
            <p class="text-xl md:text-2xl text-slate-600 mb-6 font-medium max-w-2xl mx-auto lg:mx-0 leading-relaxed">
              PawHub helps you discover adoptable pets, connect with other pet lovers, and manage your own profile in
              one place.
            </p>
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <router-link
                to="/adopt"
                class="bg-orange-500 hover:bg-orange-600 text-white font-bold px-10 py-4 rounded-full transition-all text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 text-center"
              >
                Browse Pets
              </router-link>
              ...
              <router-link
                ...
                to="/register"
                class="bg-white border-2 border-slate-200 text-slate-800 hover:border-slate-300 font-bold px-10 py-4 rounded-full transition-all text-lg shadow-sm hover:shadow text-center"
              >
                Join the Community
              </router-link>
              <router-link
          ...
          <div class="w-full lg:w-1/2 relative flex justify-center lg:justify-end mt-12 lg:mt-0">
            ...
            <div
              class="absolute inset-0 bg-sky-100 rounded-full w-[100%] h-[100%] lg:w-[120%] lg:h-[120%] lg:-translate-x-10 translate-y-10 lg:translate-x-10 lg:-translate-y-10 z-0"
            ></div>
            ...
            <img
              ...
              class="rounded-[2.5rem] shadow-2xl relative z-10 w-full max-w-xl object-cover h-[400px] sm:h-[450px] lg:h-[600px] ring-4 ring-white"
            />
          </div>
        </div>
      </div>
    ...
    <main class="pb-24">
      ...
      <section class="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <header class="mb-12 flex flex-col md:flex-row items-baseline justify-between">
          ...
          <router-link
            to="/adopt"
            class="text-sky-600 font-bold hover:text-sky-700 hover:underline mt-4 md:mt-0 flex items-center gap-1"
          >
        ...
        <div
          ...
          class="text-slate-500 bg-white p-12 rounded-3xl text-center border border-slate-100 shadow-sm"
        >
      ...
      <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div class="bg-white rounded-[3rem] p-8 md:p-12 lg:p-16 shadow-sm border border-slate-100">
          <header class="mb-12 text-center">
            ...
            <p class="text-slate-500 mt-4 text-lg font-medium max-w-2xl mx-auto">
              Read the latest tips, stories, and discussions from our members.
            </p>
          </header>
          ...
          <div
            ...
            class="text-slate-500 bg-slate-50 p-8 rounded-3xl text-center border border-slate-100"
          >
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <article
              ...
              :key="post._id"
              class="bg-slate-50 border border-slate-100 shadow-sm rounded-3xl p-8 hover:shadow-md transition-all duration-300 cursor-pointer group flex flex-col relative overflow-hidden"
              @click="$router.push(`/post/${post._id}`)"
            >
              <!-- Subtle background image for visual flair if present -->
              <div
                v-if="post.imageUrl"
                class="absolute top-0 right-0 w-full h-32 opacity-20 group-hover:opacity-40 transition-opacity duration-500 overflow-hidden pointer-events-none rounded-t-3xl mask-image-b"
              >
              ...
              <div class="relative z-10 flex-1">
                ...
                <h3
                  class="text-xl font-bold text-slate-900 mb-3 line-clamp-2 group-hover:text-sky-600 transition-colors leading-tight"
                >
                  {{ post.title }}
                </h3>
              ...
              <div
                class="mt-auto pt-5 border-t border-slate-200 text-sm font-bold text-sky-600 uppercase tracking-wide relative z-10"
              >
...
onMounted(async () => {
  try {
    ...
    featuredPets.value = pets.filter((p) => p.status === 'adoptable').slice(0, 4);
  } catch (err) {
  ...
  try {
    ...
    // Backend returns newest-first; take the latest three.
