<template>
  <article :class="cardClass" @click="router.push(`/pet/${pet._id}`)">
    ...
    <div :class="bodyClass">
      <template v-if="variant === 'featured'">
        ...
        <div class="pt-4 border-t border-slate-100 flex justify-between items-center">
          <span class="text-sky-600 font-bold group-hover:text-orange-500 transition-colors text-lg"
            >Meet {{ pet.name }} &rarr;</span
          >
        </div>
      </template>
      ...
      <template v-else>
        ...
        <p class="text-slate-500 font-medium mb-5">
          {{ pet.species }} &bull; {{ pet.breed || 'Mixed' }} &bull; {{ pet.age || '?' }} yrs
        </p>
        <button
          class="w-full bg-sky-50 text-sky-600 font-bold py-3 rounded-xl group-hover:bg-orange-500 group-hover:text-white transition-colors text-lg"
        >
          Meet {{ pet.name }}
        </button>
      </template>
...
const bodyClass = computed(() => (isFeatured.value ? 'px-2' : 'px-3 pb-2 text-center'));
</script>
